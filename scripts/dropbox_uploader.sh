...
function db_upload_file
{
    ...
    FILE_DATA="$(echo $FILE_DST | grep -o -P 'CAM_.{0,10}')"
    FILE_YEAR="$(echo ${FILE_DATA:4:4})"
    FILE_MONTH="$(echo ${FILE_DATA:9:2})"
    ...
    if [[ $TYPE != "ERR" && $SKIP_EXISTING_FILES == 1 ]]; then
        print " > Skippping already existing file \"$FILE_DST\"\n"
	sudo mv $FILE_SRC $FILE_DIR/../$VIDEO_SYNC_DIR/$FILE_DST
        return
    fi
...
function db_simple_upload_file
{
    ...
    if grep -q "^HTTP/1.1 200 OK" "$RESPONSE_FILE"; then
        print "DONE\n"
	sudo mv $FILE_SRC $(echo $FILE_SRC | sed 's/CAM.*//')/../$VIDEO_SYNC_DIR/$FILE_DST
    else
        print "FAILED\n"
